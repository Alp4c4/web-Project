...
<body background="dark-honeycomb.png">
    <div class="changepass">
        <form method="POST" action="changed.php">
            <center><legend>แก้ไขรหัสผ่าน</legend>
            <label class="prov">รหัสผ่านใหม่</label>
            <input type="text" name="password" id="password" onkeyup="check()" pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" 
            title="Must contain at least one  number and one uppercase and lowercase letter, and at least 8 or more characters" required>
            <label id="showc"></label><br>
            <label class="prov">ยืนยันรหัสผ่าน</label>
            <input type="password" id="cfpass" onkeyup="confirm()" required>
            <label id="showcf"></label><br></center>
            <button class="btchangepass">เปลี่ยนรหัสผ่าน</button>
        </form>
    </div>
</body>
</html>